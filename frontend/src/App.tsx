    ...
    <section className="flex min-h-0 flex-col bg-[#05070b]">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-slate-800 bg-[#090f18] px-4 py-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <ToolbarButton icon={study.status === "segmenting" ? Loader2 : Brain} label="MedSAM2" loading={study.status === "segmenting"} onClick={onRunSegmentation} />
          <button
      ...
      <div className="min-h-0 flex-1 p-4">
        ...
        >
          {study.previewUrl ? (
            ...
            <div
              className="medical-scan absolute inset-0 transition-transform duration-200"
        ...
        <article className="rounded-md border border-slate-800 bg-[#0c1420] p-4">
          ...
          <div className="mt-3 space-y-2">
            {study.segmentations.length === 0 ? (
              <p className="text-sm leading-6 text-slate-400">No MedSAM2 mask yet. Drag on the image to choose an ROI, or run MedSAM2 from the top control.</p>
            ) : (
