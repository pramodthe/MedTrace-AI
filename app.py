...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.caption(
            ...
            "Replacing types affects your whole Zep project—see Zep docs."
        )
        if st.button("Apply clinical ontology (project)"):
            try:
                apply_clinical_ontology(st.session_state.zep_user_id)
        ...
        if st.button(
            ...
            if not pdf_files:
                st.session_state["_pdf_err"] = "Choose at least one PDF."
            elif not skip_vlm and not (os.environ.get("NEBIUS_VL_MODEL") or "").strip():
    ...
    with col_chat:
        ...
        if user_input:
            ...
            try:
                ...
                st.session_state["_zep_ingest_err"] = f"Zep ingest failed (reply still shown): {exc}"
